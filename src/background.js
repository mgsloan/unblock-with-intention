...
var externalContentExpiry = null;
var externalContent = null;
  ...
  chrome.runtime.onMessage.addListener((request, sender, sendResponse) => {
    ...
    switch (request.type) {
      ...
      case 'GET_EXTERNAL_CONTENT': {
        ...
        if (!externalContentDirty) {
          if (externalContentExpiry && now > externalContentExpiry) {
            externalContentDirty = true;
          } else {
        ...
        if (!state.options || !state.options.url) {
          sendResponse(null);
          break;
        }
        fetch(state.options.url)
          ...
          .then(text => {
            ...
            if (password) {
              externalContent = checkAndDecrypt(text.trim(), password);
            } else {
...
function checkAndDecrypt(encryptedMsg, passphrase) {
  const encryptedHMAC = encryptedMsg.substring(0, 64);
  const encryptedBody= encryptedMsg.substring(64);
  const decryptedHMAC = CryptoJS.HmacSHA256(encryptedBody, CryptoJS.SHA256(passphrase).toString()).toString();

  if (decryptedHMAC !== encryptedHMAC) {
    // FIXME: better user messaging
    throw 'Bad passphrase!';
  }

  return decrypt(encryptedBody, passphrase);
}

const ENCRYPTION_KEYSIZE = 256;
...
/* Minified CryptoJS from staticrypt-1.3.2 */
/* eslint-disable */
